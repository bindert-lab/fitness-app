...
export function TrainingScreen({ log, onAdd, onRemove }: Props) {
  ...
  const grouped = log.reduce<Record<string, WorkoutSet[]>>((acc, entry) => {
  ...
  return (
    <div className="space-y-5 px-4 pb-28 pt-4">
      <header>
        ...
        <p className="text-[13px] text-ios-secondary">Halte fest, was du trainierst — einfach und schnell.</p>
      </header>
      ...
      <div className="rounded-card bg-ios-surface border border-ios-border p-4">
        <input
          ...
          className="w-full rounded-pill bg-ios-surface2 px-4 py-2.5 text-[15px] text-ios-label placeholder:text-ios-secondary outline-none focus:ring-2 focus:ring-ios-blue"
        />
          ...
          <div key={date}>
            ...
            <div className="space-y-2">
              {grouped[date].map((entry) => (
                ...
                >
                  <div>
                    <p className="text-[15px] font-medium text-ios-label">{entry.exercise}</p>
